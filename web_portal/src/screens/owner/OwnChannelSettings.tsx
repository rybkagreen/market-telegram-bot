...
export default function OwnChannelSettings() {
  ...
  const [formats, setFormats] = useState<FormatState>({
    post_24h: true, post_48h: true, post_7d: true, pin_24h: false, pin_48h: false,
  })
  ...
  if (isError || !settings) {
    return <Notification type="danger">Не удалось загрузить настройки канала</Notification>
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-display font-bold text-text-primary">
            @{channel?.username ?? channel?.title ?? `#${numericId}`}
          </h1>
          <p className="text-text-secondary">Настройки канала</p>
        </div>
      </div>
              ...
              <div key={key} className="flex items-center justify-between py-2">
                <div className="flex-1">
                  <div className="text-sm font-medium text-text-primary">
                    {fmt.icon} {fmt.name} <span className="text-text-tertiary">(×{fmt.multiplier})</span>
                  </div>
                  <div className="text-sm text-text-secondary tabular-nums">{formatCurrency(fmtPrice)}</div>
                  {isPin && (
      ...
      <Card title="Расписание публикаций">
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input
              label="Начало публикаций"
              type="time"
              value={publishStart}
              onChange={setPublishStart}
            />
            <Input
              label="Конец публикаций"
              type="time"
              value={publishEnd}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Button variant="secondary" onClick={() => navigate('/own/channels')}>
          ← К каналам
        </Button>
      </div>
    </div>
