...
class AnalyticsService:
    ...
    async def get_owner_stats(self, owner_id: int, session: AsyncSession) -> OwnerStats:
        ...
        earned_result = await session.execute(
            select(func.coalesce(func.sum(PlacementRequest.final_price * OWNER_SHARE), Decimal("0"))).where(
                PlacementRequest.owner_id == owner_id,
                PlacementRequest.status == PlacementStatus.published,
    ...
    async def get_public_stats(self) -> PlatformStats:
        ...
        async with async_session_factory() as session:
            try:
                # Total users
                users_result = await session.execute(select(func.count()).select_from(TelegramChat).where(TelegramChat.is_active.is_(True)))
                total_users = users_result.scalar() or 0

                # Total reach (sum of member_count)
                await session.execute(
                    select(func.sum(TelegramChat.member_count)).where(TelegramChat.is_active.is_(True))
                )
