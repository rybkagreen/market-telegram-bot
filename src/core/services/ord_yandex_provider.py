...
class YandexOrdProvider(OrdProvider):
    ...
    async def register_contract(
        self, placement_request_id: int, advertiser_ord_id: str, amount_rub: str, date_str: str
    ) -> str:
        raise NotImplementedError(self._NOT_IMPL_MSG)

    async def register_creative(
        self, placement_request_id: int, ad_text: str, media_type: str, advertiser_ord_id: str
    ) -> str:
        raise NotImplementedError(self._NOT_IMPL_MSG)

    async def report_publication(
        self, erid: str, published_at: datetime, placement_request_id: int
    ) -> bool:
        raise NotImplementedError(self._NOT_IMPL_MSG)
