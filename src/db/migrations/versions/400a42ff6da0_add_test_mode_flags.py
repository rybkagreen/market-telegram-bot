...
def upgrade() -> None:
    ...
    op.add_column(
        "telegram_chats", sa.Column("is_test", sa.Boolean(), nullable=False, server_default="false")
    )
    ...
    op.add_column(
        "placement_requests", sa.Column("test_label", sa.String(length=64), nullable=True)
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "chat_id",
            sa.Integer(),
            sa.ForeignKey("telegram_chats.id", ondelete="SET NULL"),
            nullable=True,
        ),
    ...
    # Create indexes
    op.create_index(
        "ix_mailing_logs_placement_request_id", "mailing_logs", ["placement_request_id"]
    )
    ...
    op.create_index(
        "ix_click_tracking_placement_request_id", "click_tracking", ["placement_request_id"]
    )
