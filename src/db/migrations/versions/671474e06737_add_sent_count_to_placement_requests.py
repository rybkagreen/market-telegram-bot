...
def upgrade() -> None:
    ...
    op.add_column('placement_requests', sa.Column('sent_count', sa.Integer(), server_default='0', nullable=False))
    op.add_column('placement_requests', sa.Column('failed_count', sa.Integer(), server_default='0', nullable=False))
    op.add_column('placement_requests', sa.Column('click_count', sa.Integer(), server_default='0', nullable=False))
    op.add_column('placement_requests', sa.Column('last_published_at', sa.DateTime(timezone=True), nullable=True))

    # Add missing columns to transactions
    op.add_column('transactions', sa.Column('payment_status', sa.String(length=32), nullable=True))
    op.add_column('transactions', sa.Column('balance_before', sa.Numeric(precision=12, scale=2), nullable=True))
...
def downgrade() -> None:
    ...
    op.create_index(op.f('ix_mailing_logs_status_chat'), 'mailing_logs', ['status', 'chat_id'], unique=False)
