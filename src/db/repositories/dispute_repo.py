...
class DisputeRepository(BaseRepository[PlacementDispute]):
    ...
    async def get_open(self) -> list[PlacementDispute]:
        ...
        result = await self.session.execute(
            ...
            .where(
                PlacementDispute.status.in_(
                    [DisputeStatus.open, DisputeStatus.owner_explained]
                )
            )
    ...
    async def get_by_placement(
        self,
        placement_request_id: int,
    ) -> PlacementDispute | None:
    ...
    async def get_by_user(
        self,
        user_id: int,
    ) -> list[PlacementDispute]:
        """Получить споры пользователя (как рекламодателя или владельца)."""
        result = await self.session.execute(
            select(PlacementDispute)
            .where(
                or_(
                    PlacementDispute.advertiser_id == user_id,
                    PlacementDispute.owner_id == user_id,
                )
            )
