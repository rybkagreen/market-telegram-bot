...
class PayoutRepository(BaseRepository[PayoutRequest]):
    ...
    async def get_by_owner(self, owner_id: int) -> list[PayoutRequest]:
        ...
        result = await self.session.execute(
            select(PayoutRequest).where(PayoutRequest.owner_id == owner_id).order_by(PayoutRequest.created_at.desc())
        )
    ...
    async def get_pending(self) -> list[PayoutRequest]:
        ...
        result = await self.session.execute(
            select(PayoutRequest).where(PayoutRequest.status == PayoutStatus.pending).order_by(PayoutRequest.created_at.asc())
        )
        return list(result.scalars().all())
