...
class ReputationScore(Base):
    """
    Модель репутационного счёта пользователя.
    ...
    is_advertiser_blocked: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false")
    is_owner_blocked: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false")
    advertiser_blocked_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    owner_blocked_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
