    ...
    for ch in channels:
        builder.row(
            InlineKeyboardButton(
                text=ch.get("title", "Канал"), callback_data=f"own:channel:{ch['id']}"
            )
        )
...
def channel_detail_kb(cid: int, pending: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text=f"Заявки({pending})", callback_data=f"own:channel_requests:{cid}")
    )
...
def channel_settings_kb(cid: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="Расписание", callback_data=f"own:settings:schedule:{cid}")
    )
    builder.row(
        InlineKeyboardButton(
            text="Автоподтверждение", callback_data=f"own:settings:autoaccept:{cid}"
        )
    )
...
def format_toggles_kb(cid: int, settings: dict) -> InlineKeyboardMarkup:
    ...
    for fmt_code, fmt_name in formats:
        ...
        builder.row(
            InlineKeyboardButton(
                text=f"{icon} {fmt_name}", callback_data=f"own:format:toggle:{fmt_code}:{cid}"
            )
        )
    builder.row(
        InlineKeyboardButton(text="Сохранить", callback_data=f"own:settings:formats:save:{cid}")
    )
