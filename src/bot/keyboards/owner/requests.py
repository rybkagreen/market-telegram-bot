    ...
    for req in reqs[:10]:
        builder.row(
            InlineKeyboardButton(
                text=f"Заявка #{req['id']}", callback_data=f"own:request:{req['id']}"
            )
        )
...
def request_detail_kb(rid: int, round: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="Полный текст", callback_data=f"own:request:fulltext:{rid}")
    )
...
def reject_reason_kb(rid: int) -> InlineKeyboardMarkup:
    ...
    for reason_text, reason_code in reasons:
        builder.row(
            InlineKeyboardButton(
                text=reason_text, callback_data=f"own:reject:reason:{reason_code}:{rid}"
            )
        )
