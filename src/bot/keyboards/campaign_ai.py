...
from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup, InlineKeyboardMarkup
...
def get_ai_topic_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(text="🎓 Образование", callback_data=CampaignCreateCB(step="topic_education").pack())
...
def get_ai_variants_keyboard(variants: list[str], topic: str) -> InlineKeyboardMarkup:
    ...
    # Кнопки действий
    builder.button(text="🔄 Сгенерировать заново", callback_data=CampaignCreateCB(step="regenerate").pack())
    builder.button(text="✏️ Написать свой текст", callback_data=CampaignCreateCB(step="manual_text").pack())
    builder.button(text="🔙 Назад", callback_data=CampaignCreateCB(step="back_to_menu").pack())
...
def get_campaign_editor_keyboard(
    ...
    builder.button(
        text=f"🔗 Добавить ссылку {url_status}",
        callback_data=AIEditCB(action="add_url").pack()
    )
    ...
    builder.button(
        ...
        callback_data=AIEditCB(action="confirm").pack()
    )
...
def get_quick_campaign_keyboard(variant_index: int, topic: str) -> InlineKeyboardMarkup:
    ...
    builder.button(text="🔙 К вариантам", callback_data=CampaignCreateCB(step="back_to_variants").pack())
