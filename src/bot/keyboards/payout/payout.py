...
def payout_amounts_kb(earned: Decimal) -> InlineKeyboardMarkup:
    ...
    for amount in amounts:
        if amount <= earned:
            builder.row(
                InlineKeyboardButton(text=f"{amount} ₽", callback_data=f"payout:amount:{amount}")
            )
    if earned >= 1000:
        builder.row(
            InlineKeyboardButton(text=f"Всё ({earned:.0f} ₽)", callback_data="payout:amount:all")
        )
...
def payout_confirm_kb() -> InlineKeyboardMarkup:
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Подтвердить (ввод реквизитов)", callback_data="payout:confirm")
    )
