...
def get_feedback_type_kb() -> InlineKeyboardMarkup:
    ...
    builder.button(
        text="💬 Оставить отзыв", callback_data=FeedbackCB(action="type", value="feedback")
    )
    builder.button(
        text="🐛 Сообщить об ошибке", callback_data=FeedbackCB(action="type", value="bug")
    )
    builder.button(text="💡 Предложить идею", callback_data=FeedbackCB(action="type", value="idea"))
...
def get_feedback_confirm_kb() -> InlineKeyboardMarkup:
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Отправить", callback_data=FeedbackCB(action="confirm"))
    builder.button(text="✏️ Изменить", callback_data=FeedbackCB(action="edit"))
