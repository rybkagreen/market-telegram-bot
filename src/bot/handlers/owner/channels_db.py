...
async def multi_select(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    from src.bot.keyboards.shared.channels_catalog import get_multi_select_categories_kb
...
async def show_channels_for_comparison(
    ...
    from src.bot.keyboards.advertiser.comparison import get_channel_with_compare_kb
    ...
    if len(selected) >= 2:
        from src.bot.keyboards.advertiser.comparison import get_compare_action_kb
...
async def view_channel_detail(callback: CallbackQuery, callback_data: ChannelsCB) -> None:
    ...
    from src.bot.keyboards.shared.channels_catalog import get_channel_detail_kb
...
async def start_campaign_with_channel(
    ...
    from src.bot.keyboards.advertiser.campaign import get_campaign_step_kb
...
async def toggle_tariff(
    ...
    # Перерисовать клавиатуру
    from src.bot.keyboards.shared.channels_catalog import get_multi_select_tariffs_kb
