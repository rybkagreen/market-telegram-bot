...
async def show_image_upload(target: Message | CallbackQuery, state: FSMContext) -> None:
    ...
    if isinstance(target, CallbackQuery):
        await target.message.edit_text(text, reply_markup=get_image_upload_kb())
    else:
...
async def show_confirmation(target: Message | CallbackQuery, state: FSMContext) -> None:
    ...
    if isinstance(target, CallbackQuery):
        await target.message.edit_text(confirmation_text, reply_markup=get_campaign_confirm_kb())
    else:
