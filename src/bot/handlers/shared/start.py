...
from src.bot.keyboards.shared.main_menu import (
    ...
    get_owner_menu_kb,
    get_role_menu_kb,  # Новая функция-диспетчер
)
...
async def _handle_start(message: Message, state: FSMContext, ref_code: str | None) -> None:
    ...
    if is_new and ref_code is None and user_context.role == "new":
        ...
        # Отправляем баннер с ГЛАВНЫМ меню (v3.0 архитектура)
        # /start ВСЕГДА показывает главное меню (4 кнопки)
        # Меню роли показывается только при нажатии на Кабинет
        if BANNER_PATH.exists():
            banner = FSInputFile(BANNER_PATH)
            await message.answer_photo(
                photo=banner,
...
async def handle_cabinet_command(message: Message) -> None:
    ...
    from src.bot.handlers.shared.cabinet import show_cabinet
...
async def go_to_main_menu(callback: CallbackQuery) -> None:
    """
    Вернуться в главное меню (4 кнопки, shared для всех ролей).
    Этап 0: Строгая архитектура меню.
...
async def admin_panel_redirect(callback: CallbackQuery) -> None:
    ...
    from src.bot.keyboards.admin.admin import get_admin_main_kb
...
async def feedback_redirect(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    """
    from src.bot.handlers.shared.feedback import handle_feedback_menu
...
async def go_to_my_channels(callback: CallbackQuery) -> None:
    ...
    from src.bot.handlers.owner.channel_owner import cmd_my_channels
...
async def go_to_add_channel(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    from src.bot.handlers.owner.channel_owner import cmd_add_channel
