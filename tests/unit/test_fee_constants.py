...
def test_cancel_splits_sum_to_one() -> None:
    """50% + 40% + 10% = 100%."""
    assert Decimal("1.00") == (
        CANCEL_REFUND_ADVERTISER_RATE + CANCEL_REFUND_OWNER_RATE + CANCEL_REFUND_PLATFORM_RATE
    )
...
def test_owner_net_rate_computation() -> None:
    ...
    actual = (OWNER_SHARE_RATE * (Decimal("1") - SERVICE_FEE_RATE)).quantize(Decimal("0.001"))
...
def test_platform_total_rate_computation() -> None:
    ...
    actual = (PLATFORM_COMMISSION_RATE + OWNER_SHARE_RATE * SERVICE_FEE_RATE).quantize(
        Decimal("0.001")
    )
...
def test_owner_plus_platform_equals_one() -> None:
    ...
    platform_total_rate = PLATFORM_COMMISSION_RATE + OWNER_SHARE_RATE * SERVICE_FEE_RATE
    assert (owner_net_rate + platform_total_rate).quantize(Decimal("0.001")) == Decimal("1.000")
...
def test_release_escrow_split_on_1000_rub() -> None:
    ...
    platform_commission = (final_price * PLATFORM_COMMISSION_RATE).quantize(Decimal("0.01"))
...
def test_cancel_split_on_1000_rub() -> None:
    ...
    advertiser = (final_price * CANCEL_REFUND_ADVERTISER_RATE).quantize(Decimal("0.01"))
    owner = (final_price * CANCEL_REFUND_OWNER_RATE).quantize(Decimal("0.01"))
    platform = (final_price * CANCEL_REFUND_PLATFORM_RATE).quantize(Decimal("0.01"))
