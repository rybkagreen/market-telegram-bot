...
tampering, replay window. (BL-055)"""
...
BOT_TOKEN = "1234567890:test-bot-token-redacted"
TOLERANCE = 60


def test_signed_request_verifies() -> None:
    body = b'{"telegram_id": 42, "redirect_path": "/own/payouts/request"}'
    ts, sig = sign_bot_request(body_bytes=body, bot_token=BOT_TOKEN)
...
def test_expired_timestamp_rejected() -> None:
    ...
    ts, sig = sign_bot_request(body_bytes=body, bot_token=BOT_TOKEN, timestamp_ms=long_ago)
    assert not verify_bot_request_signature(
        timestamp_header=ts,
        body_bytes=body,
        signature_header=sig,
...
def test_future_timestamp_outside_window_rejected() -> None:
    ...
    ts, sig = sign_bot_request(body_bytes=body, bot_token=BOT_TOKEN, timestamp_ms=far_future)
    assert not verify_bot_request_signature(
        timestamp_header=ts,
        body_bytes=body,
        signature_header=sig,
...
def test_inside_window_at_boundary_accepted() -> None:
    ...
    ts, sig = sign_bot_request(body_bytes=body, bot_token=BOT_TOKEN, timestamp_ms=now_ms)
...
def test_malformed_timestamp_rejected(garbage: str) -> None:
    body = b'{"x":1}'
    _ts, sig = sign_bot_request(body_bytes=body, bot_token=BOT_TOKEN)
    assert not verify_bot_request_signature(
