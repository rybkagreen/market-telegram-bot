...
class TestAPIChannelSettings:
    ...
    async def test_get_creates_defaults(
        ...
        response = await api_client_with_auth.get(f"/api/channel-settings/?channel_id={test_channel.id}")
    ...
    async def test_patch_price(
        ...
        await api_client_with_auth.get(f"/api/channel-settings/?channel_id={test_channel.id}")
