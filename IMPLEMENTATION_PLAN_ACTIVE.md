...
> Прочитать `src/db/models/legal_profile.py`, `src/api/routers/legal_profile.py`, `src/api/schemas/legal_profile.py`. Вернуть: (1) все текущие поля LegalProfile, (2) существующий enum legal_status и его значения (individual, individual_entrepreneur, self_employed, legal_entity), (3) какие поля обязательны для каждого legal_status, (4) есть ли поля `fns_verification_status`, `egrul_snapshot_at`, `inn_validated_at`.
...
> Прочитать `src/core/services/payout_service.py`, `billing_service.py`, `src/db/models/payout.py`. Вернуть: текущие payout methods (card, sbp, счёт), как связаны с legal_status owner, есть ли валидация реквизитов.
...
- G03_ADVERTISER_LEGAL_STATUS_COMPLIANT
...
- G17_VAT_OBLIGATION_HANDLED *(для legal_entity owner — счёт-фактура)*
...
- legal_status-specific логика в `G03`, `G16`, `G17`:
  ...
  - `individual_entrepreneur` → ОГРНИП валиден + ЕГРИП snapshot свежее чем N дней.
  - `legal_entity` → ОГРН валиден + ЕГРЮЛ snapshot + G17 обязателен.
...
- [ ] legal_status-matrix тесты: individual / individual_entrepreneur / self_employed / legal_entity — каждый с соответствующими required gates.
...
- [ ] **Types (TS):** те же в `web_portal/src/lib/types.ts`. В mini_app — НЕ добавляем (mini_app не рендерит gates — может содержать financial/legal_status детали).
- [ ] **Indices:**
...
- [ ] **Mini_app audit:** gates НЕ отображаются в mini_app (ПД + legal_status info). Подтверждено.
...
# Phase 4 — Supplementary Agreements / ДС (~6.5ч)

**Branch:** `feature/supplementary-agreements`

**Prerequisites:** Phase 3 merged. **ЯВНОЕ "ок" на правку `contract.py`.** Research consolidation done 2026-05-13 — see `tmp/phase4_research_summary_2026-05-13.md` for full agent A/B findings.

## 4.A Deep-dive research

Параллельно 2 агента (выполнено 2026-05-13):
...
## 4.A.1 Research locked decisions (2026-05-13)

Source: `tmp/phase4_research_summary_2026-05-13.md` (consolidated), `tmp/phase4_research_agent_a_*`, `tmp/phase4_research_agent_b_*`, `tmp/phase4_probe_state_*`.

### Marina-confirmed decisions (D1-D3)

- **D1** — G15/G16 deferred to Phase 5; Phase 4 scope = G07 + ДС flow only. Cosmetic `PHASE4_PENDING` → `PHASE5_PENDING` reclassification for G15/G16 deferred to Phase 5 prep.
- **D2** — FK rename `Contract.placement_request_id` → `placement_id` (+ relationship `placement_request` → `placement`, index `ix_contracts_placement_request_id` → `ix_contracts_placement_id`, migration via `0001_initial_schema.py` edit per BL-061 pre-prod exception).
- **D3** — Cosmetic plan corrections:
  - `button_accept` (not `click_accept` per §4.B.4 — see resolved §4.B step list below)
  - Drop "МES Acts API" docstring from G07 body (МES is acts/Sprint E/F pipeline, irrelevant to ДС)
  - `contract.metadata_json` plan claim (originally in §4.D) — **superseded by Q-A.2 ContractEvent** (field doesn't exist; closest is `legal_status_snapshot` with reserved semantics)

### Q-A resolutions (model / service / hook)

- **Q-A.1 Hook timing — Option 1.** ДС generated на owner-approval event BEFORE `→ pending_payment` transition. G07 stays in `_TRANSITION_GATES` for `(pending_owner|counter_offer) → pending_payment`. **State machine impact: none — 10 canonical statuses preserved.** UX: owner approves → handler generates ДС pair → handler attempts transition → blocks on G07 "ДС создано, подпишите" → both sign → next attempt succeeds.
- **Q-A.2 Sub-stage logging — Option 3.** New `ContractEvent` audit table (`id, contract_id FK, event_type discriminator, event_metadata JSONB, actor_user_id, created_at`). Avoids touching `contract.py` for JSONB column; future-proof for KEP request / revocation / expiry events; Pydantic discriminator schema mirrors `TransitionMetadata` pattern.
- **Q-A.3 Hook integration — Option 1.** Caller-side wrap mirror `publication_service.py:494-505` act-generation pattern. 4 sites insert `await sup_service.generate_for_placement(placement)` BEFORE `transition_service.transition(...)`. Both P1 (arbitration, advertiser counter-accept) and P2 (campaigns router, periodic auto-approve) sites integrate naturally.
- **Q-A.4 FK rename blast radius.** ~10 files Contract-scoped only. **DO NOT touch** `OrdRegistration.placement_request_id`, `Transaction.placement_request_id`, `Act.placement_request_id`, `Review.placement_request_id`, `PlacementDispute.placement_request_id` — separate FKs on other models.
- **Q-A.5 Repo additions.** All 4 net-new methods justified (`list_supplementary_for_placement`, `get_by_placement_and_role`, `count_unsigned_supplementary_for_user`, `exists_signed_supplementary_both_sides`). `role` parameter type: `Literal["owner", "advertiser"]` (no `UserRole` enum exists).
- **Q-A.6 S-48.** `SupplementaryAgreementService` Pattern 1 (`__init__(session)`, flush only, never commit). G07 body remains pure-read. Periodic task wraps `generate_for_placement` in same try/except boundary as `transition()`.

### Q-B resolutions (templates / PDF / idempotency)

- **Q-B.1 Service shape — Instance style** (mirror `ContractService`, not `ActService` classmethod). Repo nesting + delegation to `ContractService` template stack + sub-stage event logger as instance state.
- **Q-B.2 Template family — Hybrid.** Owner-side per-legal_status fan-out (4 files: `individual` / `self_employed` / `ie` / `le`); advertiser-side single file with legal_status conditionals. **Routing via dedicated `_select_template(role, legal_status)` helper in `SupplementaryAgreementService`** — DO NOT extend `_CONTRACT_TEMPLATE_MAP` (would need composite (legal_status, role) key — violates P1 isolation of existing routing). **5 new template files.**
- **Q-B.6 Idempotency.** EXISTS check by `(placement_id, role)` per side + **partial UNIQUE index** `uq_contracts_supplementary_placement_role` on `(placement_id, contract_type, role) WHERE contract_type='supplementary_agreement'` + `try/except IntegrityError` around `flush()` для race-past-EXISTS. No `expires_at` for MVP.

### Q-M decisions (Marina-confirmed 2026-05-13)

- **Q-M.1 Template partials refactor — YES** (Step 0a). Extract `_partials/contract_css.html`, `_partials/contract_header.html`, `_partials/contract_signatures.html`; refactor 12 existing contract/act templates to `{% include %}`. Rationale: 16+ CSS duplications после ДС violation Principle 4 (once-correctly).
- **Q-M.2 `PublicationFormat.label()` / `.duration_hours()` enum methods — INCLUDE** в Step 4 (PROMPT 27). General utility (also needed for UI ДС sign screen).
- **Q-M.3 `GateReason.SUPPLEMENTARY_NOT_SIGNED` — ADD.** Used in Step 7 (G07 body, PROMPT 27). Distinguishes real-fail from `PHASE4_PENDING` marker; periodic auto-approve task's `marker_codes` discriminator correctly classifies ДС-not-signed as real-fail.
- **Q-M.4 erid в ДС templates.** Placeholder "назначается до публикации". ДС generated BEFORE escrow (Q-A.1); erid generated AT escrow time.
- **Q-M.5 ContractEvent table — CONFIRMED** (Option 3 from Q-A.2).

## 4.B Implementation — 12-step ordering

Updated from research summary. Each step = single semantic commit. Effort estimate per step in research summary (total ~6.5h).

**Step 0 — Alignment commit** (`docs(phase-4): align plan with research findings`). This commit.

**Step 0a — Template partials refactor** (`refactor(templates): extract shared partials for contracts/acts (DRY)`). Q-M.1. Extract 3 partials, refactor 12 existing templates to `{% include %}`. Per P1: prevents 5 more CSS copies when ДС templates added.

**Step 1 — Schema additions** (`feat(model): supplementary agreement extensions to Contract + new ContractEvent table`).
- `src/db/models/contract.py` — D2 rename + `parent_contract_id` SELF-FK + composite `ix_contract_placement_type` + partial UNIQUE `uq_contracts_supplementary_placement_role` (Q-B.6).
- `src/db/models/contract_event.py` — NEW per Q-A.2.
- `src/api/schemas/legal_profile.py` — `ContractType` enum + Pydantic field rename.
- `src/db/migrations/versions/0001_initial_schema.py` — in-place edit per BL-061 pre-prod exception.

**Step 2 — Repo layer** (`feat(repo): ДС query methods + ContractEvent recorder helper`). 4 new methods per Q-A.5 + `record_event()` helper + `get_by_user_and_placement` param rename.

**Step 3 — Schema regen + downstream rename** (`refactor(api): rename placement_request_id → placement_id in Contract surface`). Router + ContractService + web_portal types/api + tests + snapshots (`UPDATE_SNAPSHOTS=1`). Q-A.4 scopes rename to Contract only; other models untouched.

**Step 4 — Service layer** (`feat(service): SupplementaryAgreementService + ContractEvent + PublicationFormat enum extensions`). NEW `supplementary_agreement_service.py` (instance shape per Q-B.1; `_select_template` per Q-B.2; `_build_placement_ctx` per Q-B.4); NEW `src/core/schemas/contract_event.py` (Pydantic discriminator); `PublicationFormat.label()` + `.duration_hours()` classmethods per Q-M.2.

**Step 5 — Templates** (`feat(templates): supplementary agreement HTML templates`). 5 NEW files: `supplementary_agreement_advertiser.html` + `supplementary_agreement_owner_{individual,self_employed,ie,le}.html`. erid placeholder per Q-M.4.

**Step 6 — Hook integration in 4 caller sites** (`feat(handlers): trigger ДС generation on owner approval / advertiser counter-accept`). `arbitration.py:220`, `advertiser/campaigns.py:207`, `routers/campaigns.py:329`, `notification_tasks.py:1054` — insert `await sup_service.generate_for_placement(placement)` BEFORE `transition()`. Q-A.3 caller-side wrap. Periodic task wraps in per-placement try/except (optionally SAVEPOINT).

**Step 7 — G07 body** (`feat(gates): wire real G07 body (supplementary signed both sides)`). `agreement_gates.check_g07` — pure-read via `repo.exists_signed_supplementary_both_sides(placement.id)`; drop "МES Acts API" docstring (D3). Add `GateReason.SUPPLEMENTARY_NOT_SIGNED` per Q-M.3.

**Step 8 — API endpoint for paired ДС listing** (`feat(api): GET /api/placements/{id}/supplementary-agreements`). New endpoint + `SupplementaryAgreementResponse` schema + snapshot.

**Step 9 — Frontend hook + UI sections** (`feat(web): supplementary agreement UI on CampaignWaiting + OwnRequestDetail`). `web_portal/src/api/contracts.ts` (or extend `legal.ts`); `useSupplementaryAgreement.ts`; CampaignWaiting + OwnRequestDetail sections; reuse `useSignContract`.

**Step 10 — Tests + Playwright** (`test(phase-4): unit + integration + Playwright for ДС flow`). Unit (idempotency, UNIQUE race, ContractEvent), integration (full e2e: owner approve → 2 ДС → both sign → G07 pass → transition succeeds → escrow), Playwright `sign-supplementary-agreement.spec.ts` 3 viewports.

**Step 11 — Final closure** (`docs(phase-4): CHANGES + CHANGELOG`). `reports/docs-architect/discovery/CHANGES_<date>_phase4-supplementary-agreements.md` + `CHANGELOG.md` `[Unreleased]` entry. Local merge `--no-ff` to develop per CLAUDE.md.

---

### Legacy 4.B sub-section reference (superseded by step list above)

Older 4.B.1-4.B.5 sub-sections retained as semantic reference for code review. The 12-step list above is authoritative for execution order.

### 4.B.1 Расширение `Contract` модели (semantic — see Step 1)
...
- Переименовать `placement_request_id` → `placement_id` (D2).
- Миграция в `0001_initial_schema.py` (BL-061 pre-prod exception).

### 4.B.2 `SupplementaryAgreementService` (semantic — see Step 4)
`src/core/services/supplementary_agreement_service.py` (instance shape per Q-B.1):
- `async def generate_for_placement(self, placement) -> tuple[Contract, Contract]`:
  - Создаёт пару Contract'ов (advertiser + owner) с `contract_type=supplementary_agreement`, `parent_contract_id=<framework>`, `placement_id`, `role`, `status=draft`.
  - Template: инкорпорирует условия placement (канал, цена, время, текст, erid placeholder, период) + ссылку на рамочный.
- Двух-стороннее подписание: placement считается "ДС подписано" когда оба Contract получили `status=signed`.

**BL-037 sub-stage tracking:** ДС flow sub-stages logged в `ContractEvent` (per Q-A.2 ContractEvent table): `ds_generated_advertiser`, `ds_generated_owner`, `ds_notified_both`, `ds_signed_advertiser`, `ds_signed_owner`, `ds_marked_active`.

### 4.B.3 Hook в placement flow (semantic — see Step 6, Step 7)
- ДС generated ON owner approval **BEFORE** `pending_payment` transition (per Q-A.1 Option 1).
- 4 caller sites integrate `await sup_service.generate_for_placement(placement)` BEFORE `transition()` call (per Q-A.3 Option 1).
- Gate `G07_SUPPLEMENTARY_AGREEMENT_SIGNED` body: `exists_signed_supplementary_both_sides(placement_id)` (per Step 7).

### 4.B.4 API endpoints (semantic — see Step 8)
- `GET /api/placements/{id}/supplementary-agreements` → оба ДС (advertiser + owner).
- `POST /api/contracts/{id}/sign` — уже есть, работает для ДС тоже (`button_accept` для ФЛ/самозанятых — D3 cosmetic fix; `sms_code` fallback если КриптоПро не готов).
- Уведомления: при создании ДС — notification владельцу/рекламодателю.

### 4.B.5 UI (web_portal only per ФЗ-152) (semantic — see Step 9)
- В `CampaignWaiting.tsx`: секция "Доп. соглашение — подпишите" с кнопкой перехода на ContractDetail.
- `OwnRequestDetail.tsx`: аналогично.
- Mini_app: НЕ показывает (ДС содержит реквизиты = ПД).

**Templates:** ДС templates пишутся с `_build_fee_context()` Jinja injection + `_build_placement_ctx` helper (per Q-B.4).
...
- [ ] **Sub-stage tracking (BL-037):** ~~каждый sub-stage event logged в `contract.metadata_json`~~ — **superseded by Q-A.2 ContractEvent** (см. §4.A.1 D3): новая таблица `contract_events` с дискриминатором `event_type`. `Contract.metadata_json` поле не существует; sub-stage events пишутся в `ContractEvent` через `ContractRepo.record_event()`.
